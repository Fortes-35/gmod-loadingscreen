...
function SetFilesTotal(total){ totalCalled=true; totalFiles=total; debug("Всего файлов: "+total);}
function SetFilesNeeded(needed){ if(totalCalled){ percentage = 100 - Math.round((needed/totalFiles)*100); setLoad(percentage);} }
...
$(document).ready(function(){
  ...
  var spinner=$(".spinner");
  if(spinner.length){ spinner.attr("src","images/"+Config.spinnerImage).css({width:Config.spinnerSize+"px",height:Config.spinnerSize+"px",animationDuration:Config.spinnerSpeed+"s"}); }

  if(Config.announceMessages && Config.enableAnnouncements && Config.announcementLength){
    if(Config.announceMessages.length>0){
      var i=0;
      setInterval(()=>{ announce(Config.announceMessages[i]); i++; if(i>Config.announceMessages.length-1)i=0; }, Config.announcementLength);
    }
  }
  ...
  setTimeout(()=>{
    if(!isGmod){
      ...
      setInterval(()=>{ if(needed>0){ needed--; SetFilesNeeded(needed); DownloadingFile("Файл "+needed); } },500);
